...
def activationClassifier(data, label):
    ...
    classifier = svm.SVC(C=10.0, gamma=10.0, kernel='rbf', decision_function_shape='ovo')
...
for cur in range(0,5):
    ...
    allResults[cur,0:2,:] = np.vstack((np.mean(accProned, 0), np.mean(accASL, 0)))
    allResults[cur,2:4,:] = np.vstack((np.mean(accFreetoProned, 0), np.mean(accFreetoASL, 0)))
    allResults[cur,4:6,:] = np.vstack((np.mean(accASLtoProned, 0), np.mean(accPronedtoASL, 0)))
    allResults[cur,6:8,:] = np.vstack((np.mean(accPronedRandomBase, 0), np.mean(accASLRandomBase, 0)))
    allResults[cur,8:10,:] = np.vstack((np.mean(accFreetoPronedRandomBase, 0), np.mean(accFreetoASLRandomBase, 0)))
    allResults[cur,10:12,:] = np.vstack((np.mean(accASLtoPronedRandomBase, 0), np.mean(accPronedtoASLRandomBase, 0)))
    allResults[cur,12:14,:] = np.vstack((np.mean(accPronedRandomActivation, 0), np.mean(accASLRandomActivation, 0)))
    allResults[cur,14:16,:] = np.vstack((np.mean(accFreetoPronedRandomActivation, 0), np.mean(accFreetoASLRandomActivation, 0)))
    allResults[cur,16:18,:] = np.vstack((np.mean(accASLtoPronedRandomActivation, 0), np.mean(accPronedtoASLRandomActivation, 0)))
...
np.save('NER_SVM', allResults)
plt.show()
savemat('NER_SVM.mat', mdict={'SVM': allResults})